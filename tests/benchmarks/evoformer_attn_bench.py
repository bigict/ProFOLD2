...
from profold2.model.kernel import evoformer_attn
...
def benchmark():
  ...
  for batch in range(1, 17):
    ...
    bias2 = torch.randn(batch,
                        ...
                        requires_grad=True)
    # warm up
    evoformer_attn(Q, K, V, [bias1, bias2])
    with cuda_timer(ours_fw):
      out = evoformer_attn(Q, K, V, [bias1, bias2])
  ...
  for i in range(len(ours_fw)):
    print(
        f'{i+1}\t{ours_fw[i]}\t{baseline_fw[i]}\t{ours_bw[i]}\t{baseline_bw[i]}'
    )
