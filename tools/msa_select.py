...
def process(item, dataset=None, args=None):  # pylint: disable=redefined-outer-name
  idx, c = item
...
def main(args):  # pylint: disable=redefined-outer-name
  ...
  with mp.Pool(args.processes) as p:
    for idx, pid, xlist in p.imap(
        functools.partial(process, dataset=dataset, args=args),
        enumerate(idx_list),
        chunksize=100):
      if xlist:
...
if __name__ == '__main__':
  ...
  parser.add_argument('-t', '--data', type=str, required=True,
      help='dataset')
