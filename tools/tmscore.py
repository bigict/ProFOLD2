...
def parse_tmscore_output(src, terms:tuple):
    if src[:6] == ' There':
        ...
        return result
    result = {}
    start_keyword = dict(tmscore='TM-score    = ', gdt='GDT-TS-score= ', length='Length=  ')
...
def run_scorer(predicted_path, gt_path, terms):
    scorer_process = subprocess.run([args.exe_path, predicted_path, gt_path], stdout=subprocess.PIPE)
    f = io.TextIOWrapper(io.BytesIO(scorer_process.stdout))
    if 'Deep' in os.path.basename(args.exe_path):
        # DeepScore
...
def main(args):
    ...
    for i, (pid, model_f, native_f) in enumerate(protein_ids, start=1):
        ...
        scores = run_scorer(model_f, native_f, terms=name_map.keys())
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--verbose', action="store_true", help='Get verbose information')
