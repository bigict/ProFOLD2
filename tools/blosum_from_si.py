...
def main(args):  # pylint: disable=redefined-outer-name
  ...
  state_dict_list = [torch.load(f, map_location='cpu')
                     for f in args.input_files]

  plddt_list = [confidence_from_prediction(state_dict)
                for state_dict in state_dict_list]
  profile_list = [evolution_from_prediction(state_dict)
                  for state_dict in state_dict_list]

  a = torch.cat(
      [si_from_prediction(state_dict) for state_dict in state_dict_list],
      dim=0)
  b = torch.sum(a, dim=0)
  num = torch.sum(
      rearrange(a, 'i a -> i a ()') * rearrange(a, 'i b -> i () b'),
      dim=0)
