...
class ExpressionTreeEvaluator:
  ...
  def evaluate(self, expression: str):
    ...
    def _split_tokens(expr):
      ...
      return [
          t.strip() for p in parts
          for t in re.split("\s+", p.strip()) if t.strip() != ""
      ]
...
class System:
  ...
  def _selex_eval(self, _selex_info, op: str, left, right):
    ...
    def _unpack_operands(operands, dests):
      ...
      for i, (operand, dest) in enumerate(zip(operands, dests)):
        if dest is None:
          ...
          if not (
              isinstance(operand, list) and
              all([isinstance(val, str) for val in operands])
          ):
            succ = False
            break
          unpacked[i] = operands
          ...
          if not (
              isinstance(operand, list) and all([_is_numeric(val) for val in operands])
          ):
            succ = False
            break
          ...
          if not (
              isinstance(operand, list) and all([_is_int(val) for val in operands])
          ):
            succ = False
            break
    ...
    def _parse_range(operands: list):
      ...
      if not (
          ...
          all([isinstance(opr, str) for opr in operands])
      ):
        return None
    ...
    if op in ("and", "or"):
      ...
      atom_indices = np.asarray(
          [ai.aix for ai in _selex_info["all_atoms"] for xi in ai.atom.locations()]
      )
      X_i = np.asarray(
          [
              [xi.x, xi.y, xi.z] for ai in _selex_info["all_atoms"]
              for xi in ai.atom.locations()
          ]
      )
      X_j = np.asarray(
          [
              [xi.x, xi.y, xi.z] for j in S
  ...
  def _select(
    ...
    tree = ExpressionTreeEvaluator(
        ["hyd", "all", "none"],
        [
            "not", "byres", "bychain", "first", "last", "chain", "authchain", "segid",
            "namesel", "gti", "resix", "resid", "authresid", "resname", "re", "x", "y",
            "z", "b", "icode", "name"
        ],
