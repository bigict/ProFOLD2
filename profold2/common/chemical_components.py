...
def polymer_atom_array(
    seq, seq_index, pad_with_virtual_atoms=None
) -> structure.AtomArray:
  atom_array = None  # structure.AtomArray(0)

  pad_with_virtual_atoms = default(pad_with_virtual_atoms, np.zeros(len(seq)))
  for token_idx, (restype, res_id) in enumerate(zip(seq, seq_index)):
    ...
    residue = residue_atom_array(
        residue_id, pad_with_virtual_atoms=pad_with_virtual_atoms[token_idx]
    )
