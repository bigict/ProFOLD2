...
def residue_atom_array(
    ...
    padding_with_virtual_atoms: bool = False
) -> structure.AtomArray:
  ...
  padding_length = residue_constants.atom14_type_num - atom_array.array_length()
  if padding_with_virtual_atoms and padding_length > 0:
    padding_atoms = atom_array[np.isin(atom_array.atom_name, ['CA', 'P'])]
    padding_atoms = structure.repeat(
        padding_atoms, np.stack([padding_atoms.coord] * padding_length)
    )
    atom_array = structure.concatenate([atom_array, padding_atoms])
...
def polymer_atom_array(
    seq, seq_index, padding_with_virtual_atoms: bool = False
) -> structure.AtomArray:
  atom_array = None  # structure.AtomArray(0)

  padding_with_virtual_atoms = env(
      'profold2_ccd_padding_with_virtual_atoms',
      defval=padding_with_virtual_atoms,
      dtype=int
  )
  ...
  for token_idx, (restype, res_id) in enumerate(zip(seq, seq_index)):
    ...
    residue = residue_atom_array(
        ...
        padding_with_virtual_atoms=padding_with_virtual_atoms
    )
