...
from profold2.model import atom_layout, commons, functional, diffusion, folding, evoformer
  ...
  def forward(self, headers, representations, batch):
    ...
    with torch.no_grad():
      ...
      if 'sequence' in headers:
        ...
        num_class = prob.shape[-1]
        mask = F.one_hot(batch['seq'].long(), num_classes=num_class)
  ...
  def forward(self, headers, representations, batch):
    ...
    if 'coord' in batch:
      ...
      coord, coord_mask = atom_layout.unflatten(
          batch['atom_to_token_idx'], batch['atom_padding_token_idx'], x_true, x_mask
      )
    else:
      assert 'diffusion' in headers and 'coords' in headers['diffusion']
      ...
      coord_mask = atom_layout.unflatten(
          batch['atom_to_token_idx'],
          batch['atom_padding_token_idx'],
          coord_mask=batch['ref_mask']
      )
      if exists(batch_size):
...
class SmoothLDDTHead(nn.Module):
  """Head to calc the LDDT loss.
    """
  def __init__(self, dim, padding_atom_weight=0., shard_size=None):
    super().__init__()
    del dim
  ...
  def loss(self, value, batch):
    ...
    cdist_mask = lddt_mask(x_mask)
    lddt_wrap = lambda x_pred: lddt_fn(
        torch.cdist(x_pred, x_pred), true_cdist, cdist_mask
    )
    errors = 1. - functional.sharded_apply(
        lddt_wrap, [x_pred], shard_size=shard_size, shard_dim=-3, cat_dim=-1
    )
