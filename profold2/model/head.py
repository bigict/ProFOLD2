...
class DonfidenceHead(nn.Module):
  ...
  def __init__(
    ...
    self.to_plddt = AtomwiseBinPred(dim_single, plddt_bin_num, max_atoms_per_token)
    self.to_resolved = AtomwiseBinPred(dim_single, 2, max_atoms_per_token)
  ...
  def forward(self, headers, representations, batch):
    ...
    plddt = self.to_plddt(single_repr, batch)
    presolved = self.to_resolved(single_repr, batch)

    return dict(pae=pae, pde=pde, plddt=plddt, presolved=presolved)
