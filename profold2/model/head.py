...
class SequenceDecoderHead(nn.Module):
  ...
  def forward(self, headers, representations, batch):
    ...
    coord, coord_mask = atom_layout.unflatten(
        batch['atom_to_token_idx'], batch['atom_within_token_idx'], x_true, x_mask
    )
    ...
    return dict(logits=self.to_si(single_repr))
  ...
  def loss(self, value, batch):
    ...
    labels = F.one_hot(
        batch['seq'].long(), num_classes=len(residue_constants.restypes_with_x)
    )
    errors = softmax_cross_entropy(labels=labels, logits=value['logits'])
    avg_error = functional.masked_mean(value=errors, mask=batch['mask'], epsilon=1e-6)
