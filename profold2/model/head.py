  ...
  def forward(self, headers, representations, batch):
    ...
    if 'diffusion' in headers:
      ...
      if 'coords' in headers['diffusion']:  # inference
        ...
        x_true, x_mask = map(
            lambda key: headers['diffusion'][key], ('x_denoised', 'x_mask')
        )
        if exists(batch_size):
...
class SmoothLDDTHead(nn.Module):
  """Head to calc the LDDT loss.
    """
  def __init__(
      self, dim, padding_atom_weight=0., intertoken_atom_weight=1., shard_size=None
  ):
    super().__init__()
    del dim
  ...
  def loss(self, value, batch):
    ...
    atom_to_token_idx, atom_within_token_mask = map(
        lambda key: batch[key], ('atom_to_token_idx', 'atom_within_token_mask')
    )
