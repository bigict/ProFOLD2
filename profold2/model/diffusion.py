...
class AtomAttentionEncoder(nn.Module):
  ...
  def forward(
    ...
    if exists(r_noisy) and exists(batch_size):
      atom_to_token_idx = repeat(
          atom_to_token_idx, '... i d -> ... m i d', m=batch_size
      )
...
class AtomAttentionDecoder(nn.Module):
  ...
  def forward(
    ...
    atom_to_token_idx = batch['atom_to_token_idx']
    if exists(batch_size):
      atom_to_token_idx = repeat(atom_to_token_idx, '... i -> ... m i', m=batch_size)
...
class DiffusionSampler(nn.Module):
  ...
  def forward(
    ...
    assert x_true.shape[:-1] == batch['ref_pos'].shape
    x_noisy = self.centre_random_augmenter(
        x_true, mask=x_mask[..., None], batch_size=diffuser_batch_size
    )
  ...
  def noise_scheduler(self, steps=200):
    ...
    return [
        self.diffuser.sigma_data * (s_max + (s_min - s_min) * t / steps)**self.rho
  ...
  def sample(
    ...
    for tau_idx in range(1, len(tau_list)):
      x = self.centre_random_augmenter(x, mask=x_mask[..., None])
      ...
      noise_level = torch.full(x_mask.shape[:-1], t_hat)
      if exists(diffuser_batch_size):
        noise_level = repeat(noise_level, '... -> ... m', m=diffuser_batch_size)
      ...
      dt = tau_list[tau_idx] - t_hat
      x = x_noisy + step_scale_eta * delta * dt
    return x
