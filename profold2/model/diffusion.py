...
class DiffusionSampler(nn.Module):
  ...
  def forward(
    ...
    noise = self.noise_process(
        torch.randn_like(x_noisy) * noise_level[..., None, None],
        batch,
  ...
  def noise_process(self, noise, batch, batch_size=None):
    if env('profold2_diffuser_token_level_noise', defval=0, dtype=int): # NOTE: exp
      ...
      noise = atom_layout.unflatten(atom_to_token_idx, atom_padding_token_idx, noise)
      noise = repeat(
          noise[..., ca_idx, :], '... d -> ... c d', c=residue_constants.atom14_type_num
      )
  ...
  def sample(
    ...
    x = self.noise_process(x, batch, batch_size=diffuser_batch_size)
    for tau_idx in tqdm(range(1, len(tau_list)), desc='Diffusion Sampling'):
      ...
      x_noisy = x + delta * self.noise_process(
          torch.randn_like(x), batch, batch_size=diffuser_batch_size
      )
