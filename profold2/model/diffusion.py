...
from profold2.model import atom_layout, commons, functional
...
class AtomAttentionEncoder(nn.Module):
  ...
  def forward(
    ...
    token_single_cond = functional.scatter_mean(
        atom_to_token_idx, token_single_cond, dim=-2
    )
...
class DiffusionSampler(nn.Module):
  ...
  def forward(
    ...
    x_denoised = self.diffuser(
        batch,
        x_noisy=self.noise_apply(batch, x_noisy, noise, batch_size=diffuser_batch_size),
        x_mask=x_mask,
        noise_level=noise_level,
        inputs=inputs,
        trunk_single_cond=trunk_single_cond,
  ...
  def noise_apply(self, batch, x_true, noise, batch_size=None):
    ...
    if w > 0:
      ...
      ca_noise = atom_layout.unflatten(atom_to_token_idx, atom_padding_token_idx, noise)
      ca_noise = repeat(
          ca_noise[..., ca_idx, :], '... i d -> ... i c d', c=ca_noise.shape[-2]
      )
      ca_noise = atom_layout.flatten(atom_to_token_idx, atom_within_token_idx, ca_noise)
      noise = (ca_noise * w + noise * (1 - w)) / math.sqrt(w**2 + (1 - w)**2)
    return x_true + noise
  ...
  def sample(
    ...
    x = self.noise_apply(batch, 0, x, batch_size=diffuser_batch_size)
    for tau_idx in tqdm(range(1, len(tau_list)), desc='Diffusion Sampling'):
      ...
      x_noisy = self.noise_apply(
          batch, x, delta * torch.randn_like(x), batch_size=diffuser_batch_size
      )
