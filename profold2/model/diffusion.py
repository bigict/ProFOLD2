...
class DiffusionSampler(nn.Module):
  ...
  def forward(
    ...
    noise = self.noise_apply(
        batch, torch.randn_like(x_noisy), batch_size=diffuser_batch_size
  ...
  def noise_apply(self, batch, noise, batch_size=None):
  ...
  def sample(
    ...
    for tau_idx in tqdm(range(1, len(tau_list)), desc='Diffusion Sampling'):
      ...
      x_noisy = x + delta * self.noise_apply(
          batch, torch.randn_like(x), batch_size=diffuser_batch_size
      )
