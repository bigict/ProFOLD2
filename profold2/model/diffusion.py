...
class AtomAttentionDecoder(nn.Module):
  ...
  def forward(
    ...
    atom_single_cond = self.transformer(
        atom_single_cond, context_skip, pair_skip, mask=mask, shard_size=shard_size
    )
...
class DiffusionSampler(nn.Module):
  ...
  def forward(
    ...
    x_denoised = self.diffuser(
        ...
        x_mask=x_mask[..., None, :] if exists(diffuser_batch_size) else x_mask,
        inputs=inputs,
        noise_level=noise_level,
        trunk_single_cond=trunk_single_cond,
        trunk_pair_cond=trunk_pair_cond,
  ...
  @property
  def sigma_data(self):
    return self.diffuser.sigma_data
  ...
  def noise_sampler(self, *size, batch_size=None, device=None):
    ...
    return torch.exp(self.sigma_mean + self.sigma_std * x) * self.sigma_data
  ...
  def noise_scheduler(self, steps=200):
    ...
    return [
        self.sigma_data * (s_max + (s_min - s_min) * t / steps)**self.rho
