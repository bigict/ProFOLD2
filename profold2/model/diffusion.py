...
class DiffusionConditioning(nn.Module):
  ...
  def forward(self, batch, *, trunk_single_cond, trunk_pair_cond, t_hat):
    # single conditioning
...
class DiffusionNoiseScheduler(nn.Module):
  ...
  def forward(self, steps=200, device=None, dtype=torch.float32):
    step_idx = torch.arange(steps + 1, device=device, dtype=dtype)

    s_min, s_max = self.s_min ** (1. / self.rho), self.s_max ** (1. / self.rho)
    return self.s_data * (s_max + (s_min - s_max) * step_idx / steps) ** self.rho
...
class DiffusionSampler(nn.Module):
  ...
  def forward(
    ...
    # noise
    noise = torch.randn_like(x) * sigma[..., None, None]
    # denoised_x
    self.diffuser(
        batch,
        trunk_single_cond=trunk_single_cond,
