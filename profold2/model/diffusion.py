...
class Transition(nn.Module):
  ...
  def __init__(self, dim, mult=2):
    ...
    self.proj = nn.Linear(dim, dim * mult * 2, bias=False)
    self.out = nn.Linear(dim * mult, dim, bias=False)

  def forward(self, x, shard_dim=-2, shard_size=None):
    x = self.norm(x)

    def net(x):
      x, gates = torch.chunk(self.proj(x), 2)
      return self.out(x * F.silu(gates))
...
class AtomAttentionEncoder(nn.Module):
  ...
  def forward(
    ...
    if exists(atom_noisy_emb):
      ...
      query = commons.tensor_add(
          query,
          commons.tensor_mul(
              self.from_noisy_emb(atom_noisy_emb), query_mask[..., None]
          )
      )
    ...
    query = self.transformer(
        query, query_cond, pair_cond, mask=batch['ref_mask'], shard_size=shard_size
    )
...
class AtomAttentionDecoder(nn.Module):
  ...
  def __init__(
    ...
    self.from_token = nn.Linear(dim_token_single, dim_single, bias=False)
    self.transformer = AtomTransformer(
        dim,
        depth=depth,
        heads=heads,
...
class DiffusionConditioning(nn.Module):
  ...
  def __init__(self, dim, dim_noise=256, dim_inputs=449, sigma_data=16.0):
    ...
    self.to_single = commons.residue_stack(Transition, 2, dim_single, mult=2)
    self.to_pairwise = commons.residue_stack(Transition, 2, dim_pairwise, mult=2)
  ...
  def forward(self, batch, *, noise_level, trunk_single_cond, trunk_pair_cond):
    ...
    # pair conditioning
    # TODO: concat([z_ij, RelativePositionEncoding({f^*})])
...
class DiffusionSampler(nn.Module):
  ...
  def forward(
    ...
    coord_noisy = self.rand_augmenter(
    ...
    # denoised_x
    coord_denoised = self.diffuser(
        batch,
        coord_noisy=coord_noisy,
    ...
    return coord_noisy, coord_denoised, sigma
