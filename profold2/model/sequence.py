...
def _esm_lookup(self, alphabet, tokens, clips=None):
  ...
  def pseudo_embed_positions(b):
    ...
    if pos_idx > 0:
      ...
      unks = torch.full(
          (1, pos_idx) + tokens.shape[2:],
          alphabet.unk_idx,
          dtype=tokens.dtype,
          device=tokens.device
      )
      pseudo_tokens = torch.cat(
          (tokens[b:b + 1, :1, ...], unks, tokens[b:b + 1, 1:, ...]), dim=1
      )
      pseudo_pos_embed = self.embed_positions(pseudo_tokens)
      return torch.cat(
          (pseudo_pos_embed[:, :1, ...], pseudo_pos_embed[:, pos_idx + 1:, ...]), dim=1
      )
...
class ESMEmbedding(nn.Module):
  def __init__(self, repo_or_dir, model):
    ...
    if (hasattr(self.model, 'args') and hasattr(self.model.args, 'max_positions')):
    ...
    if not self.training:  # and n > self.sequence.max_input_len:
      ...
      for k in range(0, n, max_step_len):
        ...
        if i > 0 and j < i + max_input_len:
          ...
          i = n - max_input_len
        labels = self.batch_convert([s[i:j] for s in batch['str_seq']], device=device)
        ...
        x = self.forward_(
            labels, repr_layer=ESM_EMBED_LAYER, clips=clips, return_contacts=False
        )
        p = delta // 2
        if embedds and p > 0:
          l = embedds[-1].shape[-2]
  ...
  def forward_(
      self, batch, clips=None, repr_layer=ESM_EMBED_LAYER, return_contacts=False
  ):
