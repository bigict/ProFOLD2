...
class AttentionPairBias(nn.Module):
  ...
  def forward(self, x, edges, cond=None, edge_bias=None, mask=None, edge_mask=None):
    x = self.norm(x, cond=cond)

    attn_bias = self.edges_to_attn_bias(edges)  # pylint: disable=not-callable
    if exists(edge_bias):
      attn_bias = tensor_add(attn_bias, edge_bias)
    if exists(edge_mask):
      attn_bias = attn_bias.masked_fill(~attn_mask, max_neg_value(attn_bias))
    ...
    def forward(self, *args, **kwargs):
      with profiler.record_function(moduleclass.__name__):
        for layer in self.layers:
          args = tuple(map(tensor_add, zip(args, layer(*args, **kwargs))))
        return args
  return _ResidueStack(*args, **kwargs)
