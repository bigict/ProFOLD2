...
def run(rank, args):  # pylint: disable=redefined-outer-name
  ...
  # wandb
  wandb_run = wandb_setup(args) if args.wandb_enabled and runner.is_master() else None
  if exists(wandb_run):
  ...
  loss_scaler = (
      (worker.world_size(args.nnodes) or 1) / (args.gradient_accumulate_every or 1.0)
  )
  ...
  for it in range(global_step, args.num_batches):
    ...
    if (
        args.eval_data and runner.is_master() and args.eval_every > 0 and
  ...
  if (
      ...
      (it + 1) % args.checkpoint_every != 0 and runner.is_master()
  ):
    checkpoint_manager.save(it)
  ...
  # save model
  if runner.is_master():
    runner.save(os.path.join(args.prefix, 'model.pth'), feats, model)
