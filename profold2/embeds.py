...
from profold2.utils import ESMEmbeddingExtractor, exists
from profold2.constants import MSA_MODEL_PATH, MSA_EMBED_DIM, ESM_MODEL_PATH, ESM_EMBED_DIM, PROTTRAN_EMBED_DIM
...
class MSAEmbedWrapper(nn.Module):
    def __init__(self, *, alphafold2):
        ...
        self.extractor = ESMEmbeddingExtractor(*MSA_MODEL_PATH)
    ...
    def forward(self, seq, msa, msa_mask = None, **kwargs):
        ...
        if exists(msa_mask):
            ...
            for num, batch_el in zip(num_msa, seq_and_msa_list):
                ...
                embed = self.extractor.extract(batch_el, device = device)
                embed = F.pad(embed, (0, 0, 0, 0, 0, num_rows - num), value = 0.)
                embeds.append(embed)

            embeds = torch.cat(embeds, dim = 0)
        else:
            embeds = self.extractor.extract(seq_and_msa, device = device)
...
class ESMEmbedWrapper(nn.Module):
    ...
    def forward(self, seq, msa=None, **kwargs):
        ...
        seq_embeds = self.extractor.extract(seq, model, batch_converter, device = device)
        seq_embeds = self.project_embed(seq_embeds)

        if msa is not None:
            flat_msa = rearrange(msa, 'b m n -> (b m) n')
            msa_embeds = self.extractor.extract(flat_msa, model, batch_converter, device = device)
