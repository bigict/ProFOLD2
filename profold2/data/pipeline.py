...
from profold2.data import msa_identifiers
...
from profold2.data.tools import hmmsearch
...
def make_msa_features(msas: Sequence[parsers.Msa]) -> FeatureDict:
  ...
  for msa_index, msa in enumerate(msas):
    ...
    for sequence_index, sequence in enumerate(msa.sequences):
      ...
      mol_type = residue_constants.PROT
      int_msa.append(
          [residue_constants.HHBLITS_AA_TO_ID[(res, mol_type)] for res in sequence]
      )
...
class DataPipeline:
  ...
  def process(self, input_fasta_path: str, msa_output_dir: str) -> FeatureDict:
    ...
    if self.template_searcher.input_format == 'sto':
      ...
      raise ValueError(
          'Unrecognized template input format: {self.template_searcher.input_format}'
      )
