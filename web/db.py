...
def job_get(with_tasks=True, logic_op='and', **kwargs):
  ...
  query = f'select job_id,app,sequences,email,status,time_create,time_run,time_done from jobs where {cond}'  # pylint: disable=line-too-long
...
def job_new(fasta_str, app, job_id=None, email=None):
  ...
  try:
    query = 'insert into jobs (`job_id`,`app`,`email`,`sequences`,`status`,`ip`) values (%s,%s,%s,%s,%s,%s)'  # pylint: disable=line-too-long
    if not job_id:
