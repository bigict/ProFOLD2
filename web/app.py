...
module = os.environ.get('ProFOLD_MODULE', '/serving')
...
def checking(job_id):
    ...
    job_status = dict(db.job_status(app_id))
    return render_template('checking.html', module=module,
            refresh=refresh,
            app=db.app_get(app_id),
            app_list=db.app_list(),
            job=job,
            ...
            with io.BytesIO() as f:
                with zipfile.ZipFile(f, 'w') as obj:
                    for task in job['tasks']:
                        task_zip(obj, task)
                return send_file(io.BytesIO(f.getvalue()), mimetype='application/zip, application/octet-stream',
                    as_attachment=True, attachment_filename=f'{job_id}.zip')
    else:
        if data == 'pdb':
            p = utils.serving_pdb(job_id, task_id)
            ...
            with io.BytesIO() as f:
                with zipfile.ZipFile(f, 'w') as obj:
                    task_zip(obj, task)
                return send_file(io.BytesIO(f.getvalue()), mimetype='application/zip, application/octet-stream',
                    as_attachment=True, attachment_filename=f'{job_id}_{task_id}.zip')
        else:
            return redirect(f'{module}/error?app={app_id}')
...
def error():
    app_id = request.args.get('app', 'profold0')
    job_status = dict(db.job_status(app_id))
    return render_template('error.html', module=module,
            app=db.app_get(app_id),
            job_status=job_status)
    ...
    if request.method == 'GET':
        ...
        if errors:
            return render_template('index.html', module=module,
                ...
                errors=errors)
        job_id = db.job_new(
                form.var_get(('sequences', 'sequence_file'), request.values,
                        files=request.files),
                app_id,
