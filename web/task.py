...
from web.utils import (
    serving_data, serving_log, serving_meta, serving_pdb, serving_svg
)
...
def do_task(task, uri, args, log_func=print):  # pylint: disable=redefined-outer-name
  ...
  with timing('requesting serving', log_func, prefix='   '):
    if 'params' not in task:
      task['params'] = {}
    r = requests.post(
        uri, json=dict(sequence=fasta_str, **task['params']), timeout=7200
    )
    ...
    for model_name, result in results.items():
      ...
      if 'distogram' in header:
        ...
        if args.dump_contact:
          with open(
              serving_svg(task['job_id'], task['id'], prefix=f'{model_name}_'), 'w'
          ) as f:
            f.write(svg)
      ...
      with open(
          serving_pdb(task['job_id'], task['id'], prefix=f'unrelaxed_{model_name}_'),
          'w'
      ) as f:
        f.write(pdb)
      if args.run_relaxer:
...
def do_job(job, uri, args):  # pylint: disable=redefined-outer-name
  with open(serving_log(job['job_id']), 'w') as f:
    ...
    with timing('job', log_func):
      ...
      if n == len(job['tasks']):
        db.job_set(
            job_id=job['job_id'], status=db.STATUS_DONE, time_done=datetime.now()
        )
        log_func('Job has done ...')
      else:
        db.job_set(
            job_id=job['job_id'], status=db.STATUS_ERROR, time_done=datetime.now()
        )
...
if __name__ == '__main__':
  ...
  run_job.add_argument('job_list', type=str, nargs='*', help='job list')
  ...
  run_job.add_argument('--dump_pdb', action='store_true', help='dump pdb files')
  run_job.add_argument(
      '--dump_contact', action='store_true', help='dump contact images'
  )
  run_job.add_argument(
      '--run_relaxer', action='store_true', help='dump relaxed pdb files'
  )
  run_job.add_argument('--use_gpu_relax', action='store_true', help='run relax on gpu')
  run_job.add_argument(
      '--relax_retry', type=int, default=2, help='try to run relax `n` times'
  )
  run_job.add_argument('--skip_task_done', action='store_true', help='skip tasks done')
  ...
  list_job.add_argument(
      'job_kwargs', type=str, nargs='+', help='sql where kwargs: key=value'
  )
